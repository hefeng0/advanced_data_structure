...
{
	int key = node->key;
	struct dlist_head * prev = &sl->list[MAX_LEVEL-1];
	struct dlist_head * cur = prev->next;
	for ( int i = MAX_LEVEL-1;i>=0;i-- ) {
		dlist_for_each(cur, &sl->list[i]) {
		...
		if ( i <= node->level ) {
			struct dlist_head list = INIT_LIST_HEAD(sl->list[i]);
			node->list[i] = list;
			dlist_add(&node->list[i], prev);
		}
		prev--;
		//现在prev还是sl->list[i]
		if ( i>node->level )
...
sl_node * skiplist_find(struct skiplist *sl, int key)
{
	...
	struct dlist_head * prev = &sl->list[MAX_LEVEL-1];
	for ( int i=MAX_LEVEL-1;i>=0;i-- ) {
		struct dlist_head * cur = found_level ? prev:prev->next;
		dlist_for_each(cur, &sl->list[i]) {
			sl_node * node = container_of(cur, sl_node, list[i]);
			if ( key == node->key )
				return node;
...
void skiplist_print(struct skiplist *sl)
{
	printf("----------------------------\n");
	for ( int i=MAX_LEVEL-1;i>=0;i-- ) {
		struct dlist_head * cur = sl->list[i].next;
		printf("level=%d\n", i);
		dlist_for_each(cur, &sl->list[i]) {
			sl_node *node = container_of(cur, sl_node, list[i]);
